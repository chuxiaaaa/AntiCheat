...
1\. AntiCheat 파일 디렉토리에서 locales 폴더를 찾아 localization\_cfg.json 파일을 엽니다

2\. `current\_language` 값을 수정합니다

&nbsp;  - 비어있으면 안티치트는 시스템 언어에 따라 자동으로 zh\_CN/en\_US로 설정됩니다

&nbsp;  - 지정하려면 값을 en\_US/zh\_CN/ko\_KR로 설정하세요
...
1\. en\_US.json 파일을 복사합니다

2\. 파일명을 수정하여 원하는 언어 파일명으로 변경합니다 (예: en\_US.json -> ko\_KR.json)

3\. 파일 내용을 변경하고 완료 후 `localization\_cfg.json`에서 `current\_language` 값을 수정합니다
